...
$REGEX_SVG_COLOR_STYLE = '/(fill|stroke):(.*?)(;|")/';
$REGEX_SVG_COLOR_STYLE_REPLACE = '${1}:%s${3}';
$REGEX_SVG_COLOR = '/(fill|stroke)="(#\d+)"/';
$REGEX_SVG_COLOR_REPLACE = '${1}="%s"';
$REGEX_SVG_WIDTH = '/width="(\d+)"/s';
$REGEX_SVG_HEIGHT = '/height="(\d+)"/s';
$REGEX_SVG_WIDTH_REPLACE = "width=\"%s\"";
$REGEX_SVG_HEIGHT_REPLACE = "height=\"%s\"";

$svgFolder = sprintf( preg_replace( '/\./i', DIRECTORY_SEPARATOR, "%s.image.svg" ), __DIR__ );
...
if ( $handle )
{
    ...
    while ( false !== ( $entry = readdir( $handle ) ) )
    {
        ...
        if ( basename( $entry, ".svg" ) == $svgFile )
        {
            ...
            if ( $svgColor )
            {
                $contents = preg_replace( $REGEX_SVG_COLOR_STYLE, sprintf( $REGEX_SVG_COLOR_STYLE_REPLACE, $svgColor ),
                        $contents );
